...
export async function GET(req: Request) {
    try {
        ...
        if (organizationId !== null) {
            ...
            activeClassroomIds = orgClassrooms.map((c: (typeof orgClassrooms)[number]) => c.id);
        } 
        ...
        // 3. GLOBAL FALLBACK (All user's joined classrooms)
            ...
            activeClassroomIds = userMemberships.map((m: (typeof userMemberships)[number]) => m.classroomId);
        }
        ...
        return NextResponse.json({
            ...
            topContributors: topContributors.map((c: (typeof topContributors)[number]) => ({ name: c.name, replyCount: Number(c.replyCount) })),
            classroomSettings,
